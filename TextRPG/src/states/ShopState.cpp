...
ShopState::ShopState(Party& party, Console& console, StateMachine& stateMachine, Keyboard& keyboard, const std::string& shopFilepath)
    ...
    , m_Quantity{ 0 }, m_Price{0}, m_ScreenWidth{ console.GetScreenWidth() }, m_ScreenHeight{ console.GetScreenHeight() }, m_CenterScreenW{ console.GetHalfWidth() }
    , m_PanelBarX{ m_CenterScreenW - (PANEL_BARS / 2) }, m_bInShopSelect { true }, m_bInItemBuy{false}, m_bInItemSell{false}, m_bSetFuncs{false}
